...
- Scaling-phase capture note:
  ...
  - the current committed experimental VI path combines a non-linear row-phase-aware Y adjustment (`0/2/7/18`) with an upward-skewed 4-tap vertical footprint (`upper 8/16`, `lower 7/16`), a localized `y_frac` remap for phases `1/2` in the upper source band (`y >> 10 < 640`), and experimental 4x source-domain step biases of `x_add -= 17` and `y_add -= 29`; keep that as the committed baseline unless a replacement clearly beats it
  - for local scaling sweeps, you can override those source-domain values at runtime with `PARALLEL_VI_SOURCE_{X,Y}_{ADD,BASE}_BIAS` instead of rebuilding between every probe
  - current best runtime-only source-domain candidate is `PARALLEL_VI_SOURCE_X_ADD_BIAS=17 PARALLEL_VI_SOURCE_Y_ADD_BIAS=30 PARALLEL_VI_SOURCE_Y_BASE_BIAS=736`; it materially improves the saved Paper Mario oracle, but it is not committed as the default yet because hardwiring those same values did not reproduce the runtime-override result in the helper path
  ...
  - current representative stable metrics for that experimental baseline: `full 20.0056`, `left 19.4502`, `right 30.7216`, `top 22.6831`, `bottom 21.4701`, `file2_new 2.9931`
