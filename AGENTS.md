...
- Scaling-phase capture note:
  ...
  - the current committed experimental VI path combines a non-linear row-phase-aware Y adjustment (`0/2/7/18`) with an upward-skewed 4-tap vertical footprint (`upper 8/16`, `lower 7/16`), a localized `y_frac` remap for phases `1/2` in the upper source band (`y >> 10 < 640`), and experimental 4x source-domain biases of `x_add -= 17`, `y_add -= 30`, `y_base += 736`, plus an upper-band phase-1 source Y bias of `+384` and an upper-band phase-3 source Y bias of `+512`; keep that as the committed baseline unless a replacement clearly beats it
  - for local scaling sweeps, you can override those source-domain values at runtime with `PARALLEL_VI_SOURCE_{X,Y}_{ADD,BASE}_BIAS`, and you can probe the per-phase seam controls with `PARALLEL_VI_PHASE1_Y_BIAS`, `PARALLEL_VI_PHASE3_Y_BIAS`, and `PARALLEL_VI_PHASE3_X_BIAS`
  ...
  - current representative metrics for that experimental baseline: `full 18.3457`, `left 17.5339`, `right 30.2604`, `top 17.9058`, `bottom 21.2062`, `file2_new 2.9931`
