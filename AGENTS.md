...
- Scaling-phase capture note:
  ...
  - current best runtime-only Y-origin candidate is `PARALLEL_VI_SOURCE_Y_BASE_BIAS=672`; it improves the saved Paper Mario oracle materially, but it is not committed as the default yet because the live helper path still needs one more wiring/debug pass before that default can be trusted
