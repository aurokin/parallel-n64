...
- `emu-runtime-conformance`: runtime conformance (`runtime_smoke_lavapipe` + `paper_mario_full_cache_phrb_authorities` + `paper_mario_full_cache_phrb_authorities_refresh` + `paper_mario_full_cache_phrb_authorities_zero_config_refresh` + `paper_mario_selected_package_authorities` + `paper_mario_selected_package_timeout_validation` + `paper_mario_selected_package_timeout_lookup_without_probe` + `sm64_hires_boot` + `sm64_hires_title_fixture` + `oot_hires_boot` + `oot_hires_title_fixture`) with opt-in env automatically set
- `emu-dump`: `emu.dump.*`
...
- `emu.conformance.paper_mario_full_cache_phrb_authorities`, `emu.conformance.paper_mario_full_cache_phrb_authorities_refresh`, `emu.conformance.paper_mario_full_cache_phrb_authorities_zero_config_refresh`, `emu.conformance.paper_mario_selected_package_authorities`, `emu.conformance.paper_mario_selected_package_timeout_validation`, `emu.conformance.paper_mario_selected_package_timeout_lookup_without_probe`, `emu.conformance.sm64_hires_boot`, `emu.conformance.sm64_hires_title_fixture`, `emu.conformance.oot_hires_boot`, and `emu.conformance.oot_hires_title_fixture` skip cleanly when the required local `PHRB`, ROM, savestate, or RetroArch prerequisites are absent.
- `emu.conformance.sm64_hires_boot` and `emu.conformance.oot_hires_boot` run through `tools/scenarios/cross-game-hires-boot-validation.sh`, which emits evidence bundles with final capture, runtime config, RetroArch log, extracted hi-res evidence, and `validation-summary.{json,md}` under `artifacts/cross-game-probes/validation/` by default.
- Set `EMU_RUNTIME_SM64_BUNDLE_DIR` or `EMU_RUNTIME_OOT_BUNDLE_DIR` to override where those cross-game boot evidence bundles are written.
- `emu.conformance.sm64_hires_title_fixture` and `emu.conformance.oot_hires_title_fixture` run through `tools/scenarios/cross-game-hires-savestate-fixture-validation.sh`, which verifies the fixed baseline-off savestate hash, settles three frames, captures, and writes `validation-summary.{json,md}` under `artifacts/cross-game-probes/fixtures/` by default.
- Set `EMU_RUNTIME_SM64_TITLE_FIXTURE_BUNDLE_DIR` or `EMU_RUNTIME_OOT_TITLE_FIXTURE_BUNDLE_DIR` to override where those cross-game fixture evidence bundles are written.
- The default SM64/OoT title savestates are local fixture assets under `assets/states/*-title-boot/ParaLLEl N64/`. Remint them with `tools/scenarios/remint-cross-game-boot-state.sh --expected-verify-capture-sha256 <sha256>` when the ROM/core/bootstrap flow intentionally changes; `--allow-unverified` is only for explicit exploratory remints and does not promote to the final output path.
- Cross-game fixture gates use fixed-state identity, staged-state load acknowledgement, and semantic hi-res evidence. They intentionally do not use captures as correctness checks.
- `retroarch.session.env` records adapter-requested session provenance. Runtime-observed proof still comes from RetroArch logs, load acknowledgements, extracted hi-res evidence, and capture artifacts.
- The SM64/OoT gates intentionally assert compatibility-path runtime behavior (`source_mode=phrb-only`, compat draw hits, and OoT CI hits). They are cross-game PHRB/compat regression gates, not native sampled identity proofs.
- Set `EMU_RUNTIME_PM64_FULL_CACHE_PHRB` to override the default full-cache `PHRB` conformance input; this is now the explicit way to point the shared wrapper at the zero-config compat-only artifact.
...
- The shared `paper-mario-phrb-authority-validation.sh` lane now rejects non-`.phrb` runtime inputs and validates the live `PHRB` runtime contract directly.
- The explicit selected-package timeout validation lane follows the same rule: runtime loads are `.phrb` only, while any legacy `.hts` / `.htc` use is confined to manual `hts2phrb` conversion and review tooling.
- `emu.conformance.paper_mario_selected_package_timeout_lookup_without_probe` is the narrower companion lane for the same selected package: it runs the title timeout probe directly with `PARALLEL_RDP_HIRES_SAMPLED_OBJECT_LOOKUP=1` and `PARALLEL_RDP_HIRES_SAMPLED_OBJECT_PROBE=0`, then asserts live sampled-only descriptor traffic without relying on the probe-driven review artifacts.
- Hi-res evidence and fixture verification now track the actual loaded source mix only: runtime-facing Paper Mario lanes assert `source_mode=phrb-only` and `.phrb` inputs, with no separate loader-policy contract.
- Libretro runtime loading is `.phrb` only. The removed source-policy/source-mode frontend knobs are no longer part of the supported runtime surface.
- Legacy `.hts` / `.htc` packs remain import-only tooling inputs for `hts2phrb`, review scripts, and refresh workflows; they are not supported runtime inputs.
- `emu.conformance.paper_mario_full_cache_phrb_authorities_refresh` uses a hermetic default context made from the three promoted selected-package sampled-probe authority bundles. By default it resolves `artifacts/paper-mario-probes/validation/promoted-selected-package-authorities/validation-summary.json` and skips if that promoted summary/root is absent. Set `EMU_RUNTIME_PM64_PROMOTED_CONTEXT_ROOT` to pin the promoted root, or `EMU_RUNTIME_PM64_FULL_CACHE_CONTEXT_SUMMARY` to pin the exact summary. `EMU_RUNTIME_PM64_FULL_CACHE_CONTEXT_DIR` remains an explicit non-default review override.
...
- The default full-cache conformance path now prefers the latest regenerated authority-summary-root package at [`artifacts/hts2phrb-review/20260408-pm64-all-families-authority-context-abs-summary/package.phrb`](/home/auro/code/parallel-n64/artifacts/hts2phrb-review/20260408-pm64-all-families-authority-context-abs-summary/package.phrb), then falls back to the earlier authority-context artifact, and otherwise requires an explicit `EMU_RUNTIME_PM64_FULL_CACHE_PHRB` override.
- The repo-default `paper-mario-title-screen.sh`, `paper-mario-file-select.sh`, and `paper-mario-kmr-03-entry-5.sh` authority scenarios now use that same enriched-only preference order when no cache override is provided and fail closed if none of those promoted enriched `PHRB` artifacts exist, so the normal `on` authority lane no longer silently falls back to either the zero-context compat-only artifact or the legacy `.hts` cache.
- The current live full-cache runtime-contract baseline is [`artifacts/paper-mario-probes/validation/20260408-full-cache-phrb-authorities-authority-context-abs-summary-fresh/validation-summary.json`](/home/auro/code/parallel-n64/artifacts/paper-mario-probes/validation/20260408-full-cache-phrb-authorities-authority-context-abs-summary-fresh/validation-summary.json): the promoted enriched `phrb-only` lane keeps all three authority fixtures green with mixed sampled/compat descriptor traffic (`title-screen 268/0/0/12`, `file-select 214/0/0/6`, `kmr_03 ENTRY_5 182/0/0/74`) and `native_sampled_entry_count=503`.
- The broader combined-context package under [`artifacts/hts2phrb-review/20260408-pm64-all-families-authority-plus-timeout-context-root/package.phrb`](/home/auro/code/parallel-n64/artifacts/hts2phrb-review/20260408-pm64-all-families-authority-plus-timeout-context-root/package.phrb) remains review-only: it widens the package and stays fully sampled, but it is not the active conformance baseline until the authority lane proves the promoted semantic/runtime contract with acceptable descriptor-path and fallback evidence.
- When the enriched full-cache package is the active conformance input, `emu.conformance.paper_mario_full_cache_phrb_authorities` enforces exact semantic/runtime evidence: `native_sampled_entry_count=503`, mixed sampled/compat entry class, mixed sampled/compat descriptor class, and exact descriptor-path counts for every authority fixture. When that same wrapper is pointed explicitly at the zero-config artifact through `EMU_RUNTIME_PM64_FULL_CACHE_PHRB`, it enforces `entry_class=compat-only` and `descriptor_path_class=compat-only` without treating captures as hi-res correctness proof.
- `emu.conformance.paper_mario_full_cache_phrb_authorities_refresh` goes one step further: it regenerates an enriched package from the legacy `.hts` cache plus passed authority `validation-summary.json` context, then re-runs the full authority validation. Its pass/fail contract is semantic and pinned to the current refreshed output: `context_bundle_class=context-enriched`, `package_manifest_runtime_ready_record_class=mixed-native-and-compat`, five package-level native sampled records, at least three context resolutions, explicit overlay blockers when no deterministic bindings exist, and exact authority fixture evidence of `196` native sampled runtime entries each with descriptor-path counts title `68/0/0/124`, file select `94/0/0/55`, and `kmr_03 ENTRY_5` `4/0/0/184`. Direct `hires-evidence.json` traces remain valid runtime evidence, but they are not enrichment sources for `hts2phrb --context-dir`.
- `emu.conformance.paper_mario_full_cache_phrb_authorities_zero_config_refresh` does the same for the zero-context lane: it regenerates the front-door `PHRB` from the legacy `.hts` cache alone, then re-runs the authority validation and asserts the current compat-backed runtime shape (`8992` package records, `8620` runtime-ready compat records, `372` deferred records, `context_bundle_class=zero-context`, `package_manifest_runtime_ready_record_class=compat-only`, and the current locked compat-only descriptor-path counts for the zero-config lane).
- The tracked review-only reduction lane is a maintained support-level proof, not the default baseline: [`artifacts/hts2phrb-review/20260409-pm64-all-families-authority-context-overlay-review-profile/hts2phrb-report.json`](/home/auro/code/parallel-n64/artifacts/hts2phrb-review/20260409-pm64-all-families-authority-context-overlay-review-profile/hts2phrb-report.json) currently holds `19` bindings / `9` unresolved and lowers canonical-only residue to `302` families in `75` groups with `66` tracked review selections while still passing strict full-cache authority conformance.
...
- Set `EMU_RUNTIME_PM64_SELECTED_LOADER_MANIFEST`, `EMU_RUNTIME_PM64_SELECTED_TRANSPORT_REVIEW`, or `EMU_RUNTIME_PM64_SELECTED_ALT_SOURCE_CACHE` to override the deeper timeout-lane defaults.
...
- Set `EMU_RUNTIME_PM64_SELECTED_TIMEOUT_ENFORCE_VISUAL_ENVELOPE=1` with explicit `EMU_RUNTIME_PM64_SELECTED_TIMEOUT_MIN_AE`, `EMU_RUNTIME_PM64_SELECTED_TIMEOUT_MAX_AE`, `EMU_RUNTIME_PM64_SELECTED_TIMEOUT_MIN_RMSE`, and `EMU_RUNTIME_PM64_SELECTED_TIMEOUT_MAX_RMSE` only for opt-in visual review. The default selected-package timeout conformance gates do not require hi-res-on image equality with `off`.
...
- When `EMU_RUNTIME_PM64_SELECTED_TIMEOUT_TITLE_GUARD_EVIDENCE` and `EMU_RUNTIME_PM64_SELECTED_TIMEOUT_FILE_GUARD_EVIDENCE` are available and candidate-free sampled families still exist, that same timeout lane also emits `hires-sampled-cross-scene-review.*` and folds the promotion status into `hires-runtime-seam-register.*`.
...
- For the review-only dedupe candidate [`20260407-selected-plus-timeout-960-v1-dedupe-7701ac09/package.phrb`](/home/auro/code/parallel-n64/artifacts/hires-pack-review/20260407-selected-plus-timeout-960-v1-dedupe-7701ac09/package.phrb), override those same duplicate expectations to `0` / `0`; the conformance lane then asserts semantic/runtime evidence that the `7701ac09` sampled-duplicate seam disappears while the selected-package floor, descriptor-path counts, and seam-register evidence remain valid.
- For the broader review-only asset-alias candidate [`20260407-selected-plus-timeout-960-v1-dedupe-7701ac09-asset-alias-review/package.phrb`](/home/auro/code/parallel-n64/artifacts/hires-pack-review/20260407-selected-plus-timeout-960-v1-dedupe-7701ac09-asset-alias-review/package.phrb), use those same `0` / `0` duplicate overrides plus `EMU_RUNTIME_PM64_SELECTED_TIMEOUT_PACKAGE_MANIFEST` pointed at the aliased package-manifest; the conformance lane then asserts through semantic/runtime evidence that the broader group stays package-only.
- The review-only `1b8530fb` tail-slot candidate [`20260407-selected-plus-timeout-960-v1-1b85-tail-slot-review/package.phrb`](/home/auro/code/parallel-n64/artifacts/hires-pack-review/20260407-selected-plus-timeout-960-v1-1b85-tail-slot-review/package.phrb) is not a runtime-conformance baseline.
  - It remains review-only because it intentionally converts the live `1b8530fb` pool-conflict seam into `67` sampled-duplicate keys / entries and suppresses the pool-review artifact.
