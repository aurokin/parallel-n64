...
- That path now has two meaningful parts:
  - `vi_scale.frag` keeps the tuned `0/2/7/18` row-phase schedule, the upward-skewed `upper 8/16`, `lower 7/16` 4-tap footprint, the localized `y_frac` remap for phases `1/2` when the source row is in the upper band, and now applies the phase-1 source-Y bias only in that same upper source band.
  - `scale_stage()` now also applies tested experimental source-domain biases through `vi_scale_sampling_policy`, currently `x_add -= 17`, `y_add -= 30`, `y_base += 736`, a phase-1 source Y bias of `+384`, and a phase-3-only source Y bias of `+512` for the validated 4x path.
  - local experimentation can override the global source-domain values at runtime with `PARALLEL_VI_SOURCE_{X,Y}_{ADD,BASE}_BIAS`, and can probe the per-phase seam controls with `PARALLEL_VI_PHASE1_Y_BIAS`, `PARALLEL_VI_PHASE3_Y_BIAS`, and `PARALLEL_VI_PHASE3_X_BIAS`.
  - the current committed baseline from those sweeps is `x_add -= 17`, `y_add -= 30`, `y_base += 736`, `phase1_y += 384` in the upper band, and `phase3_y += 512` in the upper band, which improved the saved Paper Mario oracle to a representative `full 18.3457 / left 17.5339 / right 30.2604 / top 17.9058 / bottom 21.2062 / file2_new 2.9931`.
...
- Latest Paper Mario oracle comparison for the current experimental path:
  - representative run: `full 18.3457`, `left 17.5339`, `right 30.2604`, `top 17.9058`, `bottom 21.2062`, `file2_new 2.9931`
  - repeated same-code captures still show a small left-side variance, which moves the `left` crop and `full` metric slightly while leaving `right`, `top`, `bottom`, and `file2_new` unchanged; for example, a repeat landed at `full 18.6343 / left 19.3481` while preserving the same `right 30.2604 / top 17.9058 / bottom 21.2062`
- Practical read on the latest improvement: source-domain mapping remains the dominant lane, but the best results now come from phase-specific source Y biases only in the upper source band. That shrinks the visible seam further without reintroducing the earlier bottom-band regression.
