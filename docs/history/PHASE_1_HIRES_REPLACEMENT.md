...
- Sampled-object miss telemetry is now split into selector-conflict misses versus unresolved misses.
  ...
  - practical implication: the active merged package no longer needs the `28916d63` clone to preserve visible output on the tracked title, file-select, or first world checkpoint
      - the first gameplay-native expansion is now validated too:
        - sampled transport review: [`20260330-timeout-960-sampled-transport/review.md`](/home/auro/code/parallel-n64/artifacts/hires-pack-review/20260330-timeout-960-sampled-transport/review.md)
    ...
    - strict title/file-select both stay pixel-identical to the selected baseline and to legacy `.hts`:
      - [`20260330-v31-add-1b85-strict-title`](/home/auro/code/parallel-n64/artifacts/paper-mario-title-screen/on/20260330-v31-add-1b85-strict-title)
      - [`20260330-v31-add-1b85-strict-file`](/home/auro/code/parallel-n64/artifacts/paper-mario-file-select/on/20260330-v31-add-1b85-strict-file)
    - non-menu validation now changes materially:
      - `960`: [`20260330-v31-legacy-timeout-960-add-1b85/validation-summary.md`](/home/auro/code/parallel-n64/artifacts/paper-mario-probes/validation/20260330-v31-legacy-timeout-960-add-1b85/validation-summary.md) improves to `AE 1659865 / RMSE 1.3327`
      - `1200`: [`20260330-v31-legacy-timeout-rest-add-1b85/validation-summary.md`](/home/auro/code/parallel-n64/artifacts/paper-mario-probes/validation/20260330-v31-legacy-timeout-rest-add-1b85/validation-summary.md) improves to `AE 3599606 / RMSE 1.8233`
      - `1500` stays visually unchanged while exact-hit coverage rises
  - practical implication: `1b8530fb` is the first gameplay pool that beats the review-only baseline strongly enough to carry into the selected native package; the next Phase 1 gameplay seam should target the next unresolved sampled family rather than revisiting menu-only selectors
- Current meaning of that result:
  ...
  - broader non-menu validation is now split into two eras:
    - the older `v19`/`v30` path proved the timeout seam was safe but visually incomplete
    - the new `1b8530fb` gameplay pool is the first native path that materially closes those non-menu gaps while keeping the menu anchors stable
  - current broader validation with the `1b8530fb` package:
    - `960` world (`kmr_03 entry 5`): [`20260330-v31-legacy-timeout-960-add-1b85/validation-summary.md`](/home/auro/code/parallel-n64/artifacts/paper-mario-probes/validation/20260330-v31-legacy-timeout-960-add-1b85/validation-summary.md) improves from `AE 123006470 / RMSE 20.2289` to `AE 1659865 / RMSE 1.3327`
    - `1200` battle (`kmr_03 entry 0`): [`20260330-v31-legacy-timeout-rest-add-1b85/validation-summary.md`](/home/auro/code/parallel-n64/artifacts/paper-mario-probes/validation/20260330-v31-legacy-timeout-rest-add-1b85/validation-summary.md) improves from `AE 60128256 / RMSE 14.5891` to `AE 3599606 / RMSE 1.8233`
    - `1500` world (`kmr_06 entry 3`) stays visually unchanged at the current selected hash, but sampled exact-hit coverage still rises
  - practical implication: the active deeper validation story is no longer “native selected packages are safe but incomplete.” It is now “the first gameplay-native pool works, and the remaining deeper gaps are narrowed to the next unresolved sampled families.”
