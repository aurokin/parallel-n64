...
Status: merged into `parallelish-reboot` 2026-07-06 after adversarial review (verdict: merge with fixes). Fix applied at merge: `input_driver = "null"` / `input_joypad_driver = "null"` moved inside the `RETROARCH_VIDEO_CONTEXT_DRIVER` conditional so the display-host append config stays byte-identical when the env is unset. The RetroArch side merges to `agent-control`. The rest of this doc is the spike-era record.
...
- Merged 2026-07-06 (see Status above); the pure-context RetroArch spike branch remains archived, unmerged.
