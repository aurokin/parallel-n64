...
tag=""
capture_root=""
savestate_dir="/tmp/parallel-n64-paper-mario-saves/intro22-seed-r1"
frame_advance="1"
...
usage() {
  cat <<'EOF_USAGE'
Usage:
  run-paper-mario-hires-intro22-state-capture.sh [options] [-- EXTRA_ARGS...]

Options:
  --tag NAME                  Capture tag to pass through
  --capture-root DIR          Override the capture root
  --savestate-dir DIR         Override the intro22 seeded savestate directory
                              (default: /tmp/parallel-n64-paper-mario-saves/intro22-seed-r1)
...
Behavior:
  - Uses the seeded intro22 same-core save state
