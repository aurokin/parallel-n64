...
EXPECTED_SHA256="88a8f0857d1b64e41c7ffcf204c3437746c116263b90e6ee30f4e9a597eb511a"

if [[ "${EMU_ENABLE_RUNTIME_CONFORMANCE:-0}" != "1" ]]; then
  echo "SKIP: set EMU_ENABLE_RUNTIME_CONFORMANCE=1 to run lavapipe VI-filter mixed frame hash conformance."
  exit 77
fi
...
parallel-n64-parallel-rdp-overscan = "8"
...
if [[ $rc -ne 0 ]]; then
  cat "$log_file"
  echo "FAIL: VI-filter mixed frame-hash run exited with status $rc." >&2
  exit 1
fi

if [[ ! -s "$shot_file" ]]; then
  cat "$log_file"
  echo "FAIL: VI-filter mixed screenshot file was not produced." >&2
  exit 1
fi

if ! grep -Eqi 'llvmpipe|lavapipe' "$log_file"; then
  cat "$log_file"
  echo "FAIL: VI-filter mixed frame-hash run did not report a software Vulkan device." >&2
  exit 1
fi

if ! grep -q 'plugin_start_gfx success\.' "$log_file"; then
  cat "$log_file"
  echo "FAIL: VI-filter mixed frame-hash run did not reach plugin_start_gfx success." >&2
  exit 1
fi

actual_sha256="$(sha256sum "$shot_file" | awk '{ print $1 }')"
if [[ "$actual_sha256" != "$EXPECTED_SHA256" ]]; then
  echo "FAIL: lavapipe VI-filter mixed frame hash mismatch." >&2
...
echo "emu_conformance_lavapipe_vi_filters_mixed_hash: PASS ($actual_sha256)"
