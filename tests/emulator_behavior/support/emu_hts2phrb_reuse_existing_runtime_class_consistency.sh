...
TMP_DIR="$(mktemp -d /tmp/parallel-n64-hts2phrb-reuse-class-consistency-XXXXXX)"
...
hts2phrb_write_synthetic_runtime_provenance "$BUNDLE_DIR" "$CACHE_PATH" "synthetic-hts2phrb-reuse-runtime-class"
...
manifest["runtime_ready_native_sampled_record_count"] = 0
manifest["runtime_ready_compat_record_count"] = manifest.get("runtime_ready_record_count", 0)
manifest["runtime_ready_record_class"] = "compat-only"
manifest["runtime_ready_record_kind_counts"] = {"canonical-sampled": 0, "compatibility-alias-family": manifest.get("runtime_ready_record_count", 0)}
...
if int(report.get("package_manifest_runtime_ready_native_sampled_record_count", 0)) != 1:
    raise SystemExit(
        f"expected rebuilt runtime-ready native-sampled count of 1, got "
...
if int(report.get("package_manifest_runtime_ready_compat_record_count", -1)) != 0:
    raise SystemExit(
        f"expected rebuilt runtime-ready compat count of 0, got "
...
if report.get("package_manifest_runtime_ready_record_class") != "native-sampled-only":
    raise SystemExit(
        f"expected rebuilt runtime-ready class of native-sampled-only, got "
        f"{report.get('package_manifest_runtime_ready_record_class')!r}"
    )
if int(package_manifest.get("runtime_ready_native_sampled_record_count", 0)) != 1:
    raise SystemExit(
        f"expected restored package-manifest native-sampled count of 1, got "
        f"{package_manifest.get('runtime_ready_native_sampled_record_count')!r}"
    )
if int(package_manifest.get("runtime_ready_compat_record_count", -1)) != 0:
    raise SystemExit(
        f"expected restored package-manifest compat count of 0, got "
        f"{package_manifest.get('runtime_ready_compat_record_count')!r}"
    )
if package_manifest.get("runtime_ready_record_class") != "native-sampled-only":
    raise SystemExit(
        f"expected restored package-manifest runtime-ready class of native-sampled-only, got "
        f"{package_manifest.get('runtime_ready_record_class')!r}"
    )
...
echo "emu_hts2phrb_reuse_existing_runtime_class_consistency: PASS"
