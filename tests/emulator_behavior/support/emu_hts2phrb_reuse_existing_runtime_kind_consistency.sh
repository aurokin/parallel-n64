...
TMP_DIR="$(mktemp -d /tmp/parallel-n64-hts2phrb-reuse-kind-consistency-XXXXXX)"
...
hts2phrb_write_synthetic_runtime_provenance "$BUNDLE_DIR" "$CACHE_PATH" "synthetic-hts2phrb-reuse-runtime-kind"
...
manifest["runtime_ready_record_kind_counts"] = {"compatibility-alias-family": manifest.get("runtime_ready_record_count", 0)}
...
if package_manifest.get("runtime_ready_record_kind_counts") != {"canonical-sampled": 1}:
    raise SystemExit(
        f"expected restored runtime-ready kind counts of canonical-sampled=1, got "
        f"{package_manifest.get('runtime_ready_record_kind_counts')!r}"
    )
...
echo "emu_hts2phrb_reuse_existing_runtime_kind_consistency: PASS"
