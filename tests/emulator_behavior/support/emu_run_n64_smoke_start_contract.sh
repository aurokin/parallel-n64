...
require_pattern "--start-key KEY      Key name to inject (default: Return)" \
...
require_pattern 'start_key="Return"' "default start key missing"
...
require_pattern 'if send_start_key "$run_pid" "$start_key" "$window_name"; then' \
