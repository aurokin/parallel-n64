...
# RetroArch round-trips the core options file on exit (rewrites it with the
# full option set), so keep an immutable launch-time snapshot for provenance.
CORE_OPTIONS_LAUNCH_FILE="$BUNDLE_DIR/core-options.launch.opt"
  ...
  case "$cmd" in
    ...
    WAIT_SAVE_STATE)
      if ! wait_for_log_pattern_after "$start_bytes" "WAIT_SAVE_STATE DONE" 30; then
        echo "[adapter] WAIT_SAVE_STATE acknowledgement missing." >&2
        exit 1
      fi
      ;;
