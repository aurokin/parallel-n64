...
usage() {
  cat <<'USAGE'
Usage:
  tools/scenarios/cross-game-hires-boot-validation.sh [options]

Required:
  --game-id ID                  Stable game label for the bundle
  --rom PATH                    ROM/content path
  --cache-path PATH             Runtime PHRB package

Options:
  --bundle-dir PATH             Bundle directory (default: artifacts/cross-game-probes/validation/<timestamp>-<game>-hires-boot)
  --seconds N                   Runtime duration before screenshot/quit (default: 30)
...
if [[ -z "$GAME_ID" || -z "$ROM_PATH" || -z "$CACHE_PATH" ]]; then
  echo "--game-id, --rom, and --cache-path are required." >&2
  exit 2
fi
if ! [[ "$RUN_SECONDS" =~ ^[0-9]+([.][0-9]+)?$ ]]; then
  echo "--seconds must be numeric." >&2
  exit 2
fi
...
for value_name in MIN_ENTRIES MIN_COMPAT_DRAW_HITS MIN_CI_ATTEMPTS MIN_CI_HITS; do
  value="${!value_name}"
  if ! [[ "$value" =~ ^[0-9]+$ ]]; then
...
if [[ -z "$BUNDLE_DIR" ]]; then
  ...
  BUNDLE_DIR="$REPO_ROOT/artifacts/cross-game-probes/validation/$(date +"%Y%m%d-%H%M%S")-${safe_game_id}-hires-boot"
fi

if (( ! REUSE )); then
  rm -rf "$BUNDLE_DIR"/captures "$BUNDLE_DIR"/logs "$BUNDLE_DIR"/traces "$BUNDLE_DIR"/savefiles
fi
mkdir -p "$BUNDLE_DIR"/captures "$BUNDLE_DIR"/logs "$BUNDLE_DIR"/traces
CACHE_SHA256="$(scenario_sha256_file "$CACHE_PATH")"
if (( ! REUSE )); then
  python3 - "$BUNDLE_DIR" "$GAME_ID" "$ROM_PATH" "$CACHE_PATH" "$CACHE_SHA256" "$CORE_PATH" "$RUN_SECONDS" "$EXPECTED_SOURCE_MODE" "$EXPECTED_ENTRY_CLASS" <<'PY'
import json
import sys
from pathlib import Path

bundle, game_id, rom_path, cache_path, cache_sha256, core_path, run_seconds, expected_source_mode, expected_entry_class = sys.argv[1:]
...
python3 - "$GAME_ID" "$ROM_PATH" "$CACHE_PATH" "$CORE_PATH" "$BUNDLE_DIR" \
  "$RUN_SECONDS" "$MIN_ENTRIES" "$MIN_COMPAT_DRAW_HITS" "$MIN_CI_ATTEMPTS" "$MIN_CI_HITS" \
  "$EXPECTED_SOURCE_MODE" "$EXPECTED_ENTRY_CLASS" "$ADAPTER_RC" <<'PY'
import hashlib
import json
import sys
from pathlib import Path
...
checks = {
    ...
    "manifest_scenario_match": manifest.get("scenario") == "cross-game-hires-boot-validation",
...
if not checks["session_command_signature_match"]:
    failures.append(
        f"Expected reused boot command signature {expected_command_signature!r}, "
...
if not checks["manifest_scenario_match"]:
    failures.append(f"Expected bundle manifest scenario cross-game-hires-boot-validation, got {manifest.get('scenario')!r}.")
...
if not checks["source_mode_match"]:
    failures.append(
        f"Expected source_mode={expected_source_mode!r}, got {summary.get('source_mode')!r}."
    )
if not checks["min_entries"]:
    failures.append(
        f"Expected entry_count >= {min_entries}, got {summary.get('entry_count')!r}."
    )
if not checks["min_compat_draw_hits"]:
    failures.append(
        f"Expected compat_draw_hits >= {min_compat_draw_hits}, got {summary.get('compat_draw_hits')!r}."
    )
if not checks["min_ci_attempts"]:
    failures.append(
        f"Expected compat_draw_ci_attempts >= {min_ci_attempts}, got {summary.get('compat_draw_ci_attempts')!r}."
    )
if not checks["min_ci_hits"]:
    failures.append(
        f"Expected compat_draw_ci_hits >= {min_ci_hits}, got {summary.get('compat_draw_ci_hits')!r}."
    )
if expected_entry_class and not checks.get("entry_class_match"):
    failures.append(
        f"Expected entry_class={expected_entry_class!r}, got {summary.get('entry_class')!r}."
    )
...
validation = {
    ...
    "capture_sha256": sha256_file(capture_path) if capture_path else None,
    "expected_source_mode": expected_source_mode,
    "expected_entry_class": expected_entry_class or None,
    "thresholds": {
...
descriptor_paths = summary.get("descriptor_path_counts") or {}
md = [
    f"# {game_id} Hi-Res Boot Validation",
    "",
    f"- Bundle: `{bundle}`",
    f"- ROM SHA-256: `{validation['rom_sha256']}`",
    f"- Cache SHA-256: `{validation['cache_sha256']}`",
