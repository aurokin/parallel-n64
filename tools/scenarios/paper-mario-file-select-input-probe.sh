...
BUNDLE_HIRES_PACK_PATH=""
BUNDLE_HIRES_PACK_SHA256="missing"
if [[ "$MODE" == "on" && -f "$PACK_PATH" ]]; then
...
cat > "$BUNDLE_DIR/bundle.json" <<EOF
{
  ...
  "inputs": {
    ...
    "hires_pack_path": "$BUNDLE_HIRES_PACK_PATH",
    "hires_pack_sha256": "$BUNDLE_HIRES_PACK_SHA256",
...
if [[ "$MODE" == "on" ]]; then
  ...
  PACK_PATH=""
  PACK_SHA256="missing"
  scenario_patch_file "$BUNDLE_DIR/bundle.json" 's|"hires_pack_path": "[^"]*"|"hires_pack_path": ""|g; s|"hires_pack_sha256": "[^"]*"|"hires_pack_sha256": "missing"|g'
  scenario_patch_file "$BUNDLE_DIR/config.env" 's|HIRES_PACK_PATH=.*|HIRES_PACK_PATH=|g'
fi
