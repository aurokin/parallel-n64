...
FIXTURE_ID="paper-mario-file-select-tile-family-probe"
MANIFEST="$REPO_ROOT/tools/fixtures/paper-mario-file-select-tile-family-probe.yaml"
MODE="on"
DRY_RUN=1
BUNDLE_DIR=""
SOURCE_BUNDLE="${SOURCE_BUNDLE:-$REPO_ROOT/artifacts/paper-mario-file-select/on/20260327-sampled-object-probe}"
RUNTIME_ENV="${RUNTIME_ENV_OVERRIDE:-$SCRIPT_DIR/paper-mario-file-select.runtime.env}"

usage() {
  cat <<'EOF'
Usage:
  tools/scenarios/paper-mario-file-select-tile-family-probe.sh [options]
...
PLAN_JSON="$BUNDLE_DIR/traces/hires-tile-family-plan.json"
REPORT_JSON="$BUNDLE_DIR/traces/hires-tile-family-report.json"
REPORT_MD="$BUNDLE_DIR/traces/hires-tile-family-report.md"
...
ADAPTER_ARGS=(
  ...
  --command "SNAPSHOT_CORE_MEMORY paper-mario-tile-family-span ${PROBE_MIN_ADDR} ${PROBE_SPAN_BYTES}" \
  --command "SCREENSHOT" \
...
python3 "$REPO_ROOT/tools/hires_block_family_probe.py" analyze \
  --plan "$PLAN_JSON" \
  --snapshot-trace "$BUNDLE_DIR/traces/paper-mario-tile-family-span.core-memory.txt" \
  --cache "$ANALYSIS_CACHE_PATH" \
  --output-json "$REPORT_JSON" \
  --output-markdown "$REPORT_MD"

echo "[scenario] tile-family probe complete: $REPORT_MD"
